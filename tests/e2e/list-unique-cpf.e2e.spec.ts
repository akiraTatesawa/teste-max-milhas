...
describe("[E2E] GET /cpf/:cpf", () => {
  ...
  describe("Success", () => {
    it("[200::OK] Should be able to list the CPF", async () => {
      const response = await server.get(`/cpf/${existingCPF}`);

      expect(response.statusCode).toEqual(httpStatus.OK);
      expect(response.body).toHaveProperty("cpf", existingCPF);
  ...
  describe("Failure", () => {
    it("[404::NOT_FOUND] Should return an error if the CPF is not in the shortlist", async () => {
      ...
      const response = await server.get(`/cpf/${fakeCPF}`);
    ...
    it("[422::UNPROCESSABLE_ENTITY] Should return an error if the cpf has an invalid format", async () => {
      ...
      const response = await server.get(`/cpf/${invalid}`);
